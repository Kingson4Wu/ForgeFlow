...
from ..rules.base import next_command
from ..rules.loader import get_rules
from ..tmux.ctl import TmuxConfig, TmuxCtl
from ..tmux.notifier import send_notification
...
class _UnchangedTracker:
    ...
    def is_unchanged_too_long(self, output: str) -> bool:
        if output != self._previous_output:
            ...
            return False
        self._count += 1
        return self._count >= UNCHANGED_OUTPUT_THRESHOLD
...
def run_monitor_mode(cfg: Config) -> int:
    ...
    unchanged_tracker = _UnchangedTracker()
    ...
    try:
        while True:
            ...
            is_processing = is_task_processing(
...
def is_task_processing(
    ...
    history: list[str],
    tracker: _UnchangedTracker | None = None,
) -> bool:
    ...
    When tracker is provided, uses it for state; otherwise uses global state (for backward compatibility).
    """
    if tracker is None:
...
_global_tracker = _UnchangedTracker()


def reset_unchanged_output_tracking() -> None:
    """Reset the tracking of consecutive unchanged outputs (backward-compatible)."""
