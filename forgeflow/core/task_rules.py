...
def check_test_failures(output: str) -> bool:
    ...
    return any(indicator in output_lower for indicator in failure_indicators) and "task completed" not in output_lower
...
def check_all_tests_passed(output: str) -> bool:
    ...
    output_lower = output.lower()
    return any(indicator in output_lower for indicator in pass_indicators)
...
def check_coverage_below_threshold(output: str, threshold: int = 80) -> bool:
    ...
    output_lower = output.lower()
    return f"coverage: {threshold}%" not in output_lower and ("coverage below threshold" in output_lower or "coverage:" in output_lower)
...
6. If tests are still failing after several attempts, consider if there's a deeper architectural issue
"""
...
def check_task_completed(output: str, config: dict[str, Any]) -> bool:
    ...
    completion_indicators = config.get("task_completion_indicators", [
        "task completed",
        "task finished", 
        "done with task",
        "finished task"
...
If you've completed the current task, respond with "Task completed" and wait for further instructions.
"""
...
def build_task_planner_rules(config: dict[str, Any]) -> list[Rule]:
    ...
    return [
        ...
        Rule(
            ...
            command=config.get("next_task_prompt", "Please proceed with the next task in the TODO list.")
        ),
        # Default task prompt
